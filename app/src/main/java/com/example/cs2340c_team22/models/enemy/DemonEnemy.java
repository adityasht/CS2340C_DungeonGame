...
public class DemonEnemy implements Enemy, PlayerCollisionSubscriber, PlayerAttackSubscriber {
    // enemy attributes
    private final int enemySprite = R.drawable.imp_idle_anim_f0;
    ...
    public void move() {
        if (!isFrozen) {
            currX += directionVectorX;
            currY += directionVectorY;
            count++;
            if (count == 5) {
                directionVectorX *= -1;
                directionVectorY *= -1;
